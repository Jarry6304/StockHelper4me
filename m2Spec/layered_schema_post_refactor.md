# 重構後分層 Schema(Bronze + Silver + Cross-Stock Cores)
...
> 4. **v1.1 新增**:標示 Cross-Stock Cores Layer 2.5 在 Bronze / Silver / M3 之間的位置(Magic Formula 為首例)
...
└─────────────────────────────────┬─────────────────────────────────────────┘
                                  │
                                  ▼
┌──────────── CROSS-STOCK CORES(Layer 2.5,v3.5 R3 新)──────────────────┐
│                                                                           │
│  Phase 8 cross-stock builders(輸入 date / 全市場 universe;不走 dirty   │
│  queue,全市場永遠重算 latest)                                            │
│                                                                           │
│  S7_cross_stock      magic_formula_ranked_derived(Greenblatt 2005       │
│  (Python builders)   EBIT/EV + ROIC cross-rank + top 30 篩選)             │
│                                                                           │
│  Future:             pairs_trading · sector_rotation · correlation_matrix│
│                                                                           │
└─────────────────────────────────┬─────────────────────────────────────────┘
                                  │ (sqlx → Rust per-stock cores)
                                  ▼
┌──────────────────────────────── M3 / CORES ──────────────────────────────┐
│   chip_cores · fundamental_cores · environment_cores                     │
│   indicator_cores · wave_cores                                            │
│   (M3 schema 由 cores_refactor_plan.md 定義;v3.5 R4 tw_cores monolith    │
│    拆 8 module:main / cli / dispatcher / writers / run_environment /     │
│    run_stock_cores / summary / helpers)                                   │
└─────────────────────────────────┬─────────────────────────────────────────┘
                                  │ (read-only query)
                                  ▼
┌──────────────── MCP / API 對外(Layer 4)───────────────────────────────┐
│   agg.query.as_of() · agg._db(connection single entry,v3.5 R5 C12)    │
│   mcp_server 5 tools · dashboards Streamlit                              │
└───────────────────────────────────────────────────────────────────────────┘
```
