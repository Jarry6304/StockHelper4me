...
> **版本**:v2.0-decisions-r2(架構決策階段,整合疊圖系統文件 + 儲存層決策)
> **日期**:2026-04-28
...
- **P1(第一版必做,9 個)**:`macd`、`rsi`、`kd`、`adx`、`ma`、`bollinger`、`atr`、`obv`、`volume`(底圖)
...
CREATE TABLE indicator_values (
    ...
    params_hash     VARCHAR(32) NOT NULL,   -- 不同參數區分
...
| 表 | 規模估算 |
|---|---|
| Raw OHLC daily | 1800 × 2500 ≈ 450 萬 row |
| indicator_values | 1800 × 2500 × 14 指標 × 平均 2 參數 × 3 時框 ≈ 3.78 億 row |
| structural_snapshots | 1800 × 2500 × 3 結構性 Core ≈ 1350 萬 row(每 row JSON 較大) |
| facts | 估約 2-5 億 row(視事件密度) |

**結論**:量在 PostgreSQL 處理範圍內(可加 partition by date / stock_id 分區優化)。
...
暖機區大小由 Core 自行宣告(例:`MA(240)` 需 240 天暖機,`MACD` 需 50 天)。

#### 14.4.2 結構性指標(Neely / Fib / SR)

每天**全量重算**,寫入 `structural_snapshots`。

理由:這類指標一根新 K 線可能改寫整段歷史解讀(例:跌破前低讓 W4 變 W1 頂),無法增量。成本可接受(1800 檔 × ~1 秒 = 30 分鐘,可並行)。

#### 14.4.3 Fact 偵測

只算「上次 Fact 截止日 → 今天」這段新事件,append-only。
...
Stage 3: Indicator Cores(滑動窗口型,並行)
    ...
    ↓
Stage 4: Structural Cores(全量重算,並行)
    ├─ Neely Core → INSERT structural_snapshots (core_name='neely_core')
    ├─ Fibonacci(消費 Neely 輸出) → INSERT structural_snapshots
    ├─ Support/Resistance → INSERT structural_snapshots
    └─ Trendline(消費 Neely monowave) → INSERT structural_snapshots
    ↓
Stage 5: Chip / Fundamental / Environment Cores(並行)
    └─ 各自讀 raw → 算 → UPSERT 對應表
    ↓
Stage 6: Fact 偵測(並行,跨所有 Core)
    └─ 偵測新事件 → INSERT facts
    ↓
[Batch 完成,DB 即真相]
```
...
| Stage 3:Indicator(14 個 × 多參數) | ~50ms | ~10 分鐘 |
| Stage 4:Neely Core | ~1-2 秒 | ~30 分鐘 |
| Stage 4:其他結構性 | ~100ms | ~5 分鐘 |
| Stage 5:Chip / Fund / Env | ~20ms | ~5 分鐘 |
| Stage 6:Fact | ~30ms | ~5 分鐘 |
| **總計** | — | **約 1 小時** |

收盤後 21:00 啟動,凌晨前完成,前端隔日早上純讀 DB。
...
當使用者請求 workflow 預設外的指標參數組合:

```
前端請求 → API 檢查 indicator_values 是否有此 params_hash
    ├─ 有 → 直接讀 DB 回傳
    └─ 無 → 觸發單檔單指標 on-demand 計算
            ├─ Core 算
            ├─ UPSERT indicator_values
            └─ 回傳結果
            └─ 註記到 workflow registry,下次 batch 自動納入
```

**頻率限制**:on-demand 補算需限速(例:每 IP 每分鐘 N 次),避免被濫用。
...
21. ✅ 結構性指標(Neely / Fib / SR / Trendline)每日全量重算
...
> **下一步**:基於本決策文件,在新副本中產出 v2.0 完整 spec(目錄結構 + 各 Core trait 定義 + Workflow toml schema + Aggregation 介面 + Storage Layer DDL + Batch Pipeline 設計 + 第一版 P0/P1 Core 詳細規格)。
