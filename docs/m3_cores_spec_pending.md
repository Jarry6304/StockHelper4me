...
### 3.7 loan_collateral_core 🟢 **decisions 拍版(2026-05-17)**

`m3Spec/chip_cores.md §十`。Bronze `loan_collateral_balance_tw` 35 cols 落地。
**11 EventKind 拍版**(5 大類 × Surge/Crash = 10 + 1 Concentration)。

| # | 拍版決策 | 依據 |
|---|---|---|
| 10.5.1 | **並存**(不整合 margin_core)| 零耦合;粒度不同(margin 6+8 vs 35) |
| 10.5.2 | **5 大類獨立 × 2 = 10 + Concentration = 11 EventKind**(對等對稱)| 加回 SettlementMargin Surge/Crash 對等 |
| 10.5.3 | **70% Concentration 閾值** | Basel Committee (2006) "Studies on Credit Risk Concentration" WP 15 — CR1 > 0.7 視為 high concentration |
| 10.5.4 | **Silver = 5 主欄 + 5 change_pct + JSONB pack 其他 25** | 對齊 margin_daily_derived;少抽象 |
| 10.5.5 | **NextDayQuota 不消費**(入 JSONB detail)| 信號性低,僅監控用 |

### 3.8 block_trade_core 🟢 **decisions 拍版(2026-05-17)**

`m3Spec/chip_cores.md §十一`。Bronze `block_trade_tw` 落地。**4 EventKind 拍版**。

| # | 拍版決策 | 依據 |
|---|---|---|
| 11.5.1 | **NO,獨立 core** | 零耦合;visible big trades(block)vs inferred net(institutional)|
| 11.5.2 | **trade_type 保留 String 不 enum** | 不抽象,按真實資料;production 5 年後 DISTINCT 看到全部再 normalize |
| 11.5.3 | **MatchingTradeSpike 80%**,production 校準 | Cao et al. (2009) "Block Trading and Stock Prices" JEF 16:1-25 — matched 通常 50-70%,>80% 異常集中 |
| 11.5.4 | **NO**(SUM 簡化)| 少抽象;MatchingSpike 已是 per-type 例外 |
| 11.5.5 | **KEEP PK,FinMind 最後一筆 wins** | 簡化;granularity 需求進 V3 backlog 加 row_idx |

### 3.9 risk_alert_core 🟢 **decisions 拍版(2026-05-17)— 搬到 chip §十二**

`m3Spec/chip_cores.md §十二`(2026-05-17 從 `environment_cores.md §十` 搬入,
拍版分層原則:per-stock signal 屬 chip)。Bronze `disposition_securities_period_tw`
落地。**4 EventKind 拍版**。

| # | 拍版決策 | 依據 |
|---|---|---|
| 10.5.1 | **不重複,Announced + Entered 並存** | 公告日 ≠ period_start(通常 + 5~7 日)|
| 10.5.2 | **escalation 60 天 ≥ 2 次** | 對齊「證券交易所公布注意交易資訊處置作業要點」§4(監管直接規則) |
| 10.5.3 | **YES,三級嚴重度(measure 中文字串 parser → metadata.severity)** | 零耦合;不同性質;Silver builder 端 regex 解析 |
| 10.5.4 | **歸 chip_cores §十二**(從 environment §十 搬入)| 分層原則:per-stock 屬 chip,environment 限全市場層 |
| 10.5.5 | **structural_snapshots NO**(facts only)| 對齊 fear_greed_core 風格 |
...
### 5.6 risk_alert_core(已搬到 chip §十二,本節廢棄 — 見 §3.9)

> 2026-05-17 user 拍版「per-stock signal 屬 chip,environment 限全市場層」分層
> 原則,搬到 `chip_cores.md §十二`。本節內容已遷移至 §3.9。

### 5.7 commodity_macro_core 🟢 **decisions 拍版(2026-05-17)— 重編 §10**

`m3Spec/environment_cores.md §十`(原 §十一,因 risk_alert 搬走重編)。Bronze
`commodity_price_daily` 落地(初版 commodity='GOLD')。**4 EventKind 拍版**。

| # | 拍版決策 | 依據 |
|---|---|---|
| 11.6.1 | **YES**,params.commodities Vec<String>(初版 ["GOLD"])| Bronze PK 已含 commodity 維度,代碼預留擴展 |
| 11.6.2 | **NO,獨立 core** | 性質不同(hard signal vs fear_greed sentiment proxy)|
| 11.6.3 | **不寫進 Core**(Aggregation 並排)| 零耦合;對齊 env §九「並排不整合」|
| 11.6.4 | **保留 all available dates,不過濾** | GOLD 23/5 國際盤,週末仍報價;對齊 exchange_rate macro 風格 |
| 11.6.5 | **streak_min_days = 5**(macro 長於個股 3)| Brock, Lakonishok & LeBaron (1992) "Simple Technical Trading Rules" JoF 47(5):1731-1764 — macro/low-freq 需 longer streak 抑 noise |

**RegimeBreak 額外**:`regime_break_window = 10`(MomentumUp/Down alternation 窗口)。
Reference: Hamilton (1989) "A New Approach to the Economic Analysis of Nonstationary
Time Series and the Business Cycle" *Econometrica* 57(2):357-384 — Markov
regime-switching 短窗 alternation pattern。
