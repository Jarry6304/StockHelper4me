...
### 3.7 loan_collateral_core 🟡 **proposal(2026-05-17,v3.20 Bronze 落地)**

`m3Spec/chip_cores.md §十` draft。Bronze `loan_collateral_balance_tw` 34 column
落地,9 個 EventKind proposal(5 大類 × 2 Surge/Crash + 1 Concentration)。

| # | 問題 | best-guess | 影響 |
|---|---|---|---|
| 10.5.1 | 與既有 margin_core 關係 | 並存(粒度不同)| 整合則 margin_core 擴 |
| 10.5.2 | 5 大類各自獨立 EventKind | YES(對等對稱)| 9 EventKind |
| 10.5.3 | LoanCategoryConcentration 70% 閾值 | best-guess | production 校準 |
| 10.5.4 | Silver 設計(5 主欄 + JSONB or 34 columns)| 5 主欄 + JSONB | 對齊 margin_daily_derived |
| 10.5.5 | NextDayQuota 是否消費 | NO(僅監控用)| 7×5=35 縮 25 |

### 3.8 block_trade_core 🟡 **proposal(2026-05-17,v3.20 Bronze 落地)**

`m3Spec/chip_cores.md §十一` draft。Bronze `block_trade_tw` 落地(PK 加 trade_type),
4 EventKind proposal。

| # | 問題 | best-guess | 影響 |
|---|---|---|---|
| 11.5.1 | 整合 institutional_core LargeTransaction? | NO(粒度不同)| 獨立 core 對齊 §四 |
| 11.5.2 | trade_type 種類完整列表 | 配對 / 鉅額 / 自營 | 5 年 backfill 後重審 |
| 11.5.3 | MatchingTradeSpike 80% 閾值 | best-guess | production 校準 |
| 11.5.4 | per-trade_type EventKind 必要 | NO(SUM 即可)| 簡化 |
| 11.5.5 | 同 (stock,date,trade_type) 多筆覆蓋 | 待 user 確認 | 可能需要 row_idx |
...
### 5.6 risk_alert_core 🟡 **proposal(2026-05-17,v3.20 Bronze 落地)**

`m3Spec/environment_cores.md §十` draft。Bronze `disposition_securities_period_tw` 落地。4 EventKind proposal(Announced / Entered / Exited / Escalation)。

| # | 問題 | best-guess | 影響 |
|---|---|---|---|
| 10.5.1 | DispositionAnnounced + Entered 重複 | 不重複 | 兩 EventKind 並存 |
| 10.5.2 | escalation 判定窗口 | 60 天 ≥ 2 次 | 校準 |
| 10.5.3 | 三級嚴重度區分(注意 / 處置 / 全額)| YES(measure parser)| 中文字串 parser |
| 10.5.4 | per-stock 訊號歸 environment 章節合理 | YES | 文件歸位 |
| 10.5.5 | structural_snapshots 寫入 | NO | facts only |

### 5.7 commodity_macro_core 🟡 **proposal(2026-05-17,v3.20 Bronze 落地)**

`m3Spec/environment_cores.md §十一` draft。Bronze `commodity_price_daily` 落地
(初版 commodity='GOLD')。4 EventKind proposal(Spike / MomentumUp / MomentumDown / RegimeBreak)。

| # | 問題 | best-guess | 影響 |
|---|---|---|---|
| 11.6.1 | 多 commodity 擴展 path | params.commodities Vec<String> | Bronze schema 已就緒 |
| 11.6.2 | 與 fear_greed_core 重複 | NO(性質不同)| 兩存,Aggregation 並排 |
| 11.6.3 | per-stock 影響整合 | 不寫進 Core | 對齊 §九「並排不整合」|
| 11.6.4 | 連假 / 週末 gold 處理 | first_per_day 不 fire | 自然跳過 |
| 11.6.5 | streak_min_days=5 vs 個股 3 | macro 容忍 noise 高 | 校準 |
