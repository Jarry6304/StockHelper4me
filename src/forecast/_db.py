...
def upsert_forecast(conn, row: dict[str, Any]) -> None:
    ...
    Optional keys:
        lower, upper, point, calibrated (default False), regime_tag, params_hash
    ...
    *,
    ...
    sql = """
        SELECT id, stock_id, forecast_date, horizon_days,
               lower, upper, point, confidence, calibrated, source_core,
...
def fetch_resolved(
    ...
    *,
    ...
    sql = """
        SELECT id, stock_id, forecast_date, horizon_days,
               lower, upper, point, confidence, calibrated, source_core,
               regime_tag, params_hash, resolved_date, realized_price,
               hit, pinball_loss
        FROM forecast_log
