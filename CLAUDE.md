...
- **v4.27**(2026-05-27)1D follow-up + 1B audit — `src/fusion/_picker.py` 抽 7 truly identical helpers(coerce_date / power_rating_* / direction_from_power / pattern_type_label / wave_count_from_label),`track1.py` + `_forecast.py` + `wave_impulse_screen.py` 三處 caller 更新 import / 55 new tests;1B probe sponsor tier 全 catalog audit(54 unused / 10 回 200 有資料)結論全 skip(對齊 v3.20 拍版 + Rust S1 後復權 convention 不替換)
- **v4.28**(2026-05-27)**B1 + 2A + B3 三 sprint 完整收尾** ☕ — b1-degree-consolidation skill 兩 blocker(degree drift + neutral behavior)整體解決(branch `claude/beautiful-cori-GZbbe`,7 commits):
  - **B1**(`6832065`)— degree single source(`src/fusion/_picker.py` 加 11-level DEGREE_ORDER/RANK + classify_degree_by_years + canonical_is_invalidated;track1 / neely_emitter 刪 local 改 import;_forecast delegate)+ write-side closure(emit_neely_fib 加 current_price + 7-day stale gate + canonical_is_invalidated filter,main.py CLI 補 fetch_latest_close)+ alembic `h4i5j6k7l8m9` 加 `forecast_log.logic_version` TEXT NOT NULL(既有 row backfill 'pre_b1' / 新寫入 'b1' / 已 settle row CASE guard 凍結);修舊 bug Minor=Intermediate 同 rank=4 + SubMicro/Micro 落 0;+91 tests
  - **2A**(`d451583` scaffold + `186c3e9` hotfix + `2c8cf39` v4.28-buffer + `2912b64` v4.28-rr_cap)— wave_impulse_screen 5 threshold(後加 1)抽 `ScreenThresholds` dataclass + parametrize 4 helper + as-of `_fetch_*` + 新 `compute_screen_at_date()` read-only;新 `src/cross_cores/wave_impulse_calibrate.py` hygiene calibration harness(build_threshold_combos cartesian / build_date_series / aggregate_hygiene_metrics / calibrate_hygiene matrix iteration / samples_to_csv_rows);CLI `python src/main.py wave-impulse-calibrate --start-date/--end-date/--step-days/--sweep axis=v1,v2,.../--use-default-sweep/--output`;+30 tests
  - **2A calibration sequence**(production verify-driven,2026-05-27)— Path A 走 structural_snapshots WHERE snapshot_date ≤ asof:
    - 5/20 single-date buffer sweep(4-cell):buffer 0.03→0.05→0.07→0.10,outlier 10/7/6/9,p50 12.3/7.8/6.2/4.2 → **5% 為 metric-stability 最佳**
    - 5/20 buffer × min_upside joint sweep(6-cell):min_upside 邊際效應幾乎為 0,不動 → `CORRECTION_BOTTOM_BUFFER 0.03 → 0.05`(`2c8cf39`)
    - Production verify(`2c8cf39` 後)揭露 **timeframe stratification**:daily/weekly 乾淨(max RR 12-13),**monthly degree 仍 RR 38-465 outlier**(top 6 razor-thin stop_pct 0-0.6%)→ 加 `RR_MAX_CAP = 20.0` 通用 threshold(`2912b64`)
    - Production final state:daily 28 cands / max RR 12.63 / 0 capped;weekly 56 / 13.43 / 0 capped;monthly **33** / **19.61** / **6 capped**(從 39 - 6 razor)— rr_max 從 r5 估 5000+ → r7 1780 → v4.28-buffer 465 → v4.28-rr_cap **19.61**(-98.9%),candidates 從 25 → 33(+32%)
  - **Calibration harness 改進 user-side calibration workflow**:既有 calibration plan 過於依賴 user 手調 module 常數重跑 production;v4.28 後 `python src/main.py wave-impulse-calibrate --sweep ...` 可隨時跑歷史 replay,不影響 production wave_impulse_screen_derived 表(read-only)
  - **B3**(`4ee8e09`)— 讀取面 predicate 統一到 canonical_is_invalidated(對齊 b1 skill「禁 direction-blind fallback」+ final output reliability)。User 拍版選 A(neutral 不濾):
    - `track1.py:384-386` neutral branch:`relevant_thresholds = all_thresholds` → `[]`(對齊 canonical `_check_any_threshold_breached([], cp)` 永遠 False;v4.25.x 「neutral 走 ALL kinds」自此退役)
    - `mcp_server/_forecast.py:_scenario_is_invalidated` 從 direction-blind ANY-kind logic 改 delegate `canonical_is_invalidated`(MCP `neely_forecast` v3.35 picker filter 與 `dual_track_resonance` A-3 gate 對同 scenario 結論一致)
    - 不變:`_extract_invalidation_price`(display-only)/ `_extract_all_invalidation_thresholds` / `_check_any_threshold_breached`(低層 helper,wave_impulse_screen 仍 import)/ `scenario_is_invalidated`(backward-compat)/ `resonance.py` / MCP `dual_track_resonance` tool(透明)
    - 行為改變只影響 power_rating=Neutral 的 scenario(0050 / ETF / 中性個股 LLM context 從錯誤的「neutral A-3 invalidated」→ 正確的「mode: neutral, no directional thesis」)
    - tests +9 cross-tool consistency(parametrized bullish/bearish/neutral × below/above × breach/no-breach 驗 track1.invalidated == canonical_is_invalidated;regression-lock single source of truth)
  - **❌ Retract「`cross_tf_aligned = 0` audit」backlog**(2026-05-27 同日確認):當時 calibration 8 samples(asof 04-01 ~ 05-20)觀察 cross_tf=0 → 誤判全 production 都 0。實際追查 production current state(asof 05-21,1 天後)揭露 **44 stocks cross_tf_aligned=True**(daily 4 / weekly 18 / monthly 3 candidates;total intersection_same_dir=44 全對齊)。原因:weekly snapshots production 累積比 daily 慢 → 5/20 calibration replay 時 weekly 還沒累積足夠 → intersection=0;5/21 production current 時 weekly 累積到 tipping point → intersection=44。**屬 production snapshot 累積動態,非 code bug**,既有 `_apply_cross_tf_alignment` + `TestCrossTFAlignment` sandbox tests 正確;無需 V2 audit。
- **v4.29**(2026-05-28)13-tool MCP toolkit 3-bug fix + silver `--builder` flag(branch `claude/nice-feynman-wpW8l` PR #110,4 commits / +36 tests):
  - **Bug B**(`879787c`):`indicators` lookback_days 真實切 series — `src/fusion/indicator_assembly.py:92-97` 加 Python-level tail-slice;MACD 單 core ~182KB → ~5KB,5-core default preset payload 從 ~910KB → ~50KB(避開 MCP 1MB context limit)
  - **Bug C**(`43b0f86`):`dual_track_resonance` N+1 query 修法 — `src/fusion/dual_track/track2.py` 新 `fetch_bands_batch` 單 SQL 取 3 horizons × 6 source_core(`ANY(%s::int[])` + `ANY(%s::text[])` 顯式 cast),取代既有 18 query;`mcp_server/tools/data.py` 加 `SET LOCAL statement_timeout = '30s'` 安全網 + QueryCanceled graceful error response;對 2330 timeout > 4 min → < 1s
  - **Bug A**(`b2096b3`):`monthly_revenue` Silver builder 自 PR #18.5 邏輯錯誤 — `revenue_yoy = row.get("revenue_year")` 把 FinMind calendar year(2026.0)當 YoY% 用。fundamental_forecast.py 既有 `_compute_yoy_3m_avg` 強證據 + M8 v4.24 production verify 反證確認 `revenue_year`/`revenue_month` 是日曆年月。修法:builder 內 `_build_period_index` 建 (market, stock_id, year, month) → revenue index,per-row 算 `(rev - rev_prev_year_same_month) / base * 100`;`_fetch_all_bronze_rows` bypass v4.15 incremental 180d READ window(YoY 需 13+ 月 history;體量 ~110k rows 全 fetch < 1s);3 個下游 tool(`stock_snapshot.fundamentals` / `monthly_screen.revenue_momentum` / `monthly_trigger_scan.positive_triggers`)同時修好,Silver schema 不變
  - **feat `--builder`**(`26e6d27`):silver phase 7a/7b 加 `--builder NAME[,NAME2]` flag(對齊 cross_cores phase 8 既有 pattern);`SilverOrchestrator._filter_builders` 校驗(unknown name 早 raise ValueError 不靜默吃);Bug A verify 從跑全 phase 7a 80 分鐘 → 跑單一 monthly_revenue ~25 秒
  - **verify harness**:新 `scripts/verify_mcp_toolkit_v4_29.py` 全覆蓋 13 個 public MCP tool 健康度檢查(per-stock 6 + market-level 7,payload size budget + sanity 欄位 + per-tool summary),退碼 0/1
  - **MCP toolkit surface 清乾淨**:`mcp_server/server.py` 13 active mcp.tool() + 12 已 comment-out 舊 helper(v3.31 6 個 stock_snapshot 合併進 + v3.30 6 個 render tool 暫關),不需要再清理
...
python src/main.py backfill                              # 全量回補(估 ~30h @ 5700 reqs/h sponsor tier;v4.14 後 incremental ~13 分鐘走 all_market)
...
pydantic),壓縮交 brotli-asgi(brotli q4 + gzip_fallback)。**sync** psycopg ConnectionPool(lifespan)+ sync
handler(FastAPI threadpool)— production verify 揭露 Windows + Python 3.14 async psycopg 撞 ProactorEventLoop
不相容,改 sync pool 走 thread 阻塞 I/O 與 event loop 解耦,跨 OS/Python 穩定(commit 39ec85d 後 hotfix)。
...
- 2.25s/req × (probe + 可能 fallback 4.5s) × 57 ≈ **4-5 分鐘**(probe script 自帶 pacing,即使 5700/h sponsor tier 仍走 script 內 sleep — 不會擠 rate limit)
