...
> 最後更新：2026-04-30（v1.7）

---

## 分支狀態

- **開發分支**：`claude/review-collector-dependencies-n03rE`（已推到 origin；累積 review #3 + #4 共 8 個 commit）
...
- **PR**：v1.7 已開（review #3 + #4），待 base 維護者 / Codex / Cursor review
...
1. **PR review + merge**（v1.7 PR 已開，等 base 維護者 / Codex / Cursor review）
2. **agent-review-mcp 支線開始**（spec 在最早的訊息）
3. **Phase 4 incremental 優化**（在 Python 層做 dirty-detection 跳過未變動股票，可省每天 ~6 分鐘）
4. **inspect_db.py 升 PG 版**（v2.0 後該支腳本已不可用，但 user 不見得會碰；優先序低）
5. **CLAUDE.md 章節重組**（每輪 review 都加段落，文件愈來愈長，下次可考慮把 v1.4 / v1.5 / v1.6 commits 與詳解搬到 docs/ 目錄）
