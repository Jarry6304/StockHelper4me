...
> `traditional_core`(原列 P3)從未實作,Fusion Layer P2.2 從清單移除(vaporware:
> `rust_compute/cores/wave/` 僅有 `neely_core`)。
...
> `kalman_filter_core` 位於 `cores/indicator/` 目錄但獨立計數(1-D Kalman regime,非傳統 indicator);詳見 `kalman_filter_core.md`。

### 8.3 Chip Cores(8 個)

| Core | 文件 |
|---|---|
| `institutional_core` / `margin_core` / `foreign_holding_core` / `shareholder_core` / `day_trading_core` / `loan_collateral_core` / `block_trade_core` / `risk_alert_core` | `chip_cores.md` |
...
> `magic_formula_core` Rust crate(`cores/fundamental/`)與 Python `cross_cores/magic_formula.py`(§8.7)**分工互補,皆為 live**:Python builder 算跨股 ranking 寫 `magic_formula_ranked_derived`;Rust core 讀該表產 per-stock facts / indicator_values(由 `tw_cores` dispatch)。Fusion Layer P2.1 查證後不移除。

### 8.5 Environment Cores(7 個)

| Core | 文件 |
|---|---|
| `us_market_core` / `taiex_core` / `exchange_rate_core` / `fear_greed_core` / `market_margin_core` / `business_indicator_core` / `commodity_macro_core` | `environment_cores.md` |

### 8.6 System Cores

| Core | 職責 |
|---|---|
| `fusion_layer`(原 `aggregation_layer`) | 即時請求路徑核心。雙端口:Raw 端口並排不整合(`fusion.raw`)+ Integration 端口跨 core 整合不引入新規則(見 `fusion_layer.md`) |
| `orchestrator` | Workflow 編排,Core 註冊發現、依序呼叫、結果組裝 |

System Core 的詳細規格屬「即時路徑層」,不在本系列(Core 計算層)文件範圍。

> **「並排呈現,不整合」原則**:自 Fusion Layer(`fusion_layer.md`)起,此原則僅適用 Raw 端口(`fusion.raw`);Integration 端口允許跨 core 整合,但不引入新規則。
...
### 8.7 Cross-Stock Cores(Layer 2.5,11 個)

跨股 ranking / 分群,Python 實作於 `src/cross_cores/`,走 Phase 8 排程(全市場重算,不走 dirty queue)。

| Toolkit | Core(builder) |
|---|---|
| 獨立 | `magic_formula` |
| A(monthly) | `persistent_momentum` / `revenue_momentum` / `institutional_concert` |
| B(quarterly) | `f_score` / `low_volatility` / `industry_adj_gp` |
| C(annual) | `long_term_low_vol` / `dividend_yield` / `mom_12_1` |
| Layer 5(monthly overlay) | `monthly_trigger` |
...
| **P3** | 進階指標與離線模組 | `ichimoku_core` / `williams_r_core` / `cci_core` / `coppock_core` / `keltner_core` / `donchian_core` / `vwap_core` / `mfi_core` / Learner 離線模組 |
...
- ✅ Fusion Layer · Raw 端口並排呈現,使用者自己看出「布林收進 Keltner 內」
