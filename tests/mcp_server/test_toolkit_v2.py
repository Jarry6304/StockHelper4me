...
class TestPayloadSize:
    ...
    def test_three_tool_chain_under_15k_tokens(self, monkeypatch):
        ...
        # 3) neely_forecast(v4.39 dossier 路徑;fixtures 見 test_dossier)
        from tests.mcp_server.test_dossier import _patch_tool, make_neely_routes

        _ = ma_indicator  # dossier 路徑不再吃 indicator snapshot
        _patch_tool(monkeypatch, make_neely_routes(
            n_candidates=3, tree_depth=1, weekly_monthly_candidates=1,
        ))
